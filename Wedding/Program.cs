...
namespace Wedding
{
   internal class Program
   {
      ...
      {
         try {
            InlineKeyboardMarkup keyboard = new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithUrl("Открыть в картах", "https://yandex.ru/maps/-/CCUWaEvDLC") } });
            Connect.LoadUser(users);
            for (int i = 0; i < users.Count; i++)
               await client.SendTextMessageAsync(users[i].id, "Привет-привет!😘🥂\n\nСвадебный сунцовенок на связи, мы уже говорили, что приглашаем тебя на наш праздник любви, смеха и безумия, поэтому приколы начинаются уже до свадьбы😊 Сообщаем, что празднование переносится в ресторан «Рыба моя», зал WellWell по адресу: Славянская площадь, 2 в 17:00.\n\nТакже, если ты планируешь присоединиться к нам в ЗАГСе(адрес: Москва 1 - й Красногвардейский пр - д, 21 стр. 2, 84 этаж), то лучше подъезжать к 15:20 - 15:30, так как сейчас работает только один лифт, и время ожидания увеличивается.\n\nВ ЗАГСе будет работать координатор Яна (+7 (977) 681-17-41), которая поможет сориентироваться на территории Москва сити и встретит тебя у ЗАГСа.", replyMarkup: keyboard);
         } catch { }
      }
            ...
            if (data == "NoVisit") {
               ...
               if (data.Contains("One") || data.Contains("Two") || data.Contains("Three")) {
                  ...
                  if (data.Contains("One")) {
                     ...
                     await client.SendTextMessageAsync(message.Chat.Id, "Отлично, ждем тебя в ЗАГСе Birds на 84 этаже к 15:30, после росписи поедем в бар \"Моя рыба\" на Славянской площади, 2", replyMarkup: keyboard);
                     await Task.Delay(200);
                     await client.DeleteMessageAsync(message.Chat.Id, message.MessageId);
                  }
                  else if (data.Contains("Two")) {
                     Connect.Query("update `User` set visit = 'Бар' where id = '" + message.Chat.Id + "';");
                     InlineKeyboardMarkup keyboard = new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithUrl("Открыть в картах", "https://yandex.ru/maps/-/CCUWaEvDLC") }, new[] { InlineKeyboardButton.WithCallbackData("Далее", "11Text") } });
                     await client.SendTextMessageAsync(message.Chat.Id, "Супер, тогда приезжай сразу в бар \"Рыба моя\" на Славянской площади, 2 к 17:00", replyMarkup: keyboard);
                     await Task.Delay(200);
                     await client.DeleteMessageAsync(message.Chat.Id, message.MessageId);
                  }
               ...
               InlineKeyboardMarkup keyboard = new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithUrl("ЗАГС в Birds", "https://yandex.ru/maps/org/175522552121") }, new[] { InlineKeyboardButton.WithUrl("Бар \"Рыба моя\"", "https://yandex.ru/maps/-/CCUWaEvDLC") }, new[] { InlineKeyboardButton.WithUrl("Вишлист 💝", url: "https://mywishboard.com/collection/wedding-wishlist-1144138") }, new[] { InlineKeyboardButton.WithUrl("Дресс-код для девочек 🐆", url: "https://ru.pinterest.com/chizhuliya/girls-dress-code-moodboard/?invite_code=ec74c929c3a24755bba643f72eacc6e7&sender=375558193834645871") }, new[] { InlineKeyboardButton.WithUrl("Дресс-код для мальчиков 🦣", url: "https://ru.pinterest.com/chizhuliya/boys-dress-code-moodboard/?invite_code=2e3ccf3a119648fdbe6efa8b76352032&sender=375558193834645871") }, new[] { InlineKeyboardButton.WithUrl("Написать Владу", url: "https://t.me/vladislav_sapunov") }, new[] { InlineKeyboardButton.WithUrl("Написать Даше", url: "https://t.me/dadaborik") }, new[] { InlineKeyboardButton.WithUrl("Написать Артуру", url: "https://t.me/ArthurCarter") } });
               await client.SendTextMessageAsync(message.Chat.Id, "Свадьба проходит в Москве 1 апреля🥂\n\nСбор в ЗАГСе в Birds на 84 этаже в 15:30 💍\n\nЧеловек, который встретит и поможет в случае чего - Яна +7 (977) 681-17-41\n\nДля тех, кто сразу едет в бар \"Рыба моя\" время сбора 17:00 💃🏽\n\nЧеловек, который встретит и решит любой вопрос - Ася +7 (977) 681-17-41\n\nЕсли есть сюрпризы, творческие заготовки итд пиши скорее Владу 🐒\n\nПо всем другим вопросам пиши нашим организаторам Даше и Артуру 🐳", replyMarkup: keyboard);
            }
            else if (data == "1Text") {
               try {
               ...
               try {
                  InlineKeyboardMarkup map = new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithUrl("Открыть в картах", "https://yandex.ru/maps/-/CCUWaEvDLCИфк") } });
                  await client.EditMessageReplyMarkupAsync(message.Chat.Id, message.MessageId, replyMarkup: map);
               } catch { }
               await client.SendTextMessageAsync(message.Chat.Id, "С таймингом опредлились, давай разберемся с концепцией мероприятия\n\nЧто тебе нужно знать о нем, чтобы подготовиться и взять от этого дня максимум эмоций и впечатлений - три простых вещи:");
               await Task.Delay(500);
               ...
               await client.SendTextMessageAsync(message.Chat.Id, "Первая часть начинается в 15:30 в ЗАГСе Birds - башня Око. После росписи мы отправляемся на вечернюю часть в бар \"Рыба моя\" на Славянской площади, 2 и там уже говорится, пока силы не покинут нас :)");
